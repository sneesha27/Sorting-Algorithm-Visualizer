...
void display_text()
{
	glColor3f(0.08,0.03,0.10);
	bitmap_output(150, 695, "DYNAMIC SORTING ALGORITHM VISUALIZER",GLUT_BITMAP_TIMES_ROMAN_24);
	...
	bitmap_output(50, 625, "This program sorts a random set of numbers in ascending order displaying them graphically as ",GLUT_BITMAP_9_BY_15);
	bitmap_output(190, 605, "bars with varying height",GLUT_BITMAP_9_BY_15);


	if (sorting == 0)	// if not sorting display menu
	{
		bitmap_output(10, 575, "**   MENU   **",GLUT_BITMAP_9_BY_15);
		bitmap_output(10, 555, "1. Press s to SORT",GLUT_BITMAP_9_BY_15);
		bitmap_output(10, 535, "2. Press c to SELECT the sort algorithm",GLUT_BITMAP_9_BY_15);
		bitmap_output(10, 515, "3. Press r to RANDOMISE",GLUT_BITMAP_9_BY_15);
		bitmap_output(10, 495, "4. Press Esc to QUIT",GLUT_BITMAP_9_BY_15);
...
void front()
{
	...
	//	glVertex2f(395, 560);
	//glEnd();
	bitmap_output(330, 515, "TO",GLUT_BITMAP_TIMES_ROMAN_24);
	//glBegin(GL_LINE_LOOP);
		//glVertex2f(325, 521);
		//glVertex2f(360, 521);
	//glEnd();

	bitmap_output(150, 465, "DYNAMIC SORTING ALGORITHM VISUALIZER",GLUT_BITMAP_TIMES_ROMAN_24);
	//glBegin(GL_LINE_LOOP);
	//	glVertex2f(145, 470);
	//	glVertex2f(520, 470);
	//glEnd();

	glColor3f(0.0,0.5,0.80);
	glBegin(GL_QUADS);
		glVertex2f(520,100.0);
		glVertex2f(520,170);
		glVertex2f(796,170);
		glVertex2f(796,100.0);
	glEnd();
...
void Initialize() {
	...
	glClearColor(186.0f/255.0f,216.0f/255.0f,226.0f/255.0f,0.0f/255.0f);
...
void display()
{
	...
	if(k==0)
		...
		for(ix=0;ix<MAX;ix++)
		{
			glColor3f(0.0f/255.0f,0.0f/255.0f,139.0f/255.0f);
			glBegin(GL_LINE_LOOP);
		...
		{
			glColor3f(139.0f/255.0f,0.0f/255.0f,0.0f/255.0f);
			glBegin(GL_POLYGON);
